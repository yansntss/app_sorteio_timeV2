...
export async function onRequestGet({ params, env, data }) {
  ...
  const { results: confirmations } = await env.DB.prepare(`
    SELECT gc.id, gc.user_id, gc.guest_name, gc.confirmed_at, gc.stars, gc.goals, gc.assists,
...
export async function onRequestPatch({ params, request, env, data }) {
  ...
  // Apply stars/goals/assists to player_profiles when the game is finished for the first time
  if (body.status === "finished" && game.status !== "finished" && !game.rating_applied_at) {
    const now = Date.now();

    const { results: confs } = await env.DB.prepare(`
      SELECT gc.stars, gc.goals, gc.assists, COALESCE(u.name, gc.guest_name) as name
      FROM game_confirmations gc
      LEFT JOIN users u ON u.id = gc.user_id
      WHERE gc.game_id = ? AND (gc.stars IS NOT NULL OR gc.goals IS NOT NULL OR gc.assists IS NOT NULL)
    `).bind(id).all();
    ...
    for (const c of confs) {
      ...
      const existing = await env.DB.prepare(
        "SELECT id, avg_rating, vote_count, total_goals, total_assists FROM player_profiles WHERE owner_id = ? AND name = ?"
      ...
      if (existing) {
        ...
        await env.DB.prepare(`
          ...
          SET avg_rating = ?, vote_count = ?, total_goals = ?, total_assists = ?, updated_at = ?
          WHERE id = ?
        `).bind(newAvg, newCount, existing.total_goals + goalsAdd, existing.total_assists + assistsAdd, now, existing.id).run();
      } else {
        await env.DB.prepare(`
          INSERT INTO player_profiles (id, owner_id, name, avg_rating, vote_count, total_goals, total_assists, created_at, updated_at)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).bind(
          newSessionId(), game.owner_id, c.name,
          c.stars != null ? c.stars : 3.0, c.stars != null ? 1 : 0,
          goalsAdd, assistsAdd, now, now
        ).run();
      }
    }
